...
echo "install git and other tools"
...
buildah run rust_development2 /bin/sh -c 'apt install -y apt-utils'
...
# TODO: the command `complete` cannot be run before the container is built: 
# complete -C "dev_bestia_cargo_completion" cargo'
# probably I have to put it in bashrc
...
buildah run rust_development2 /bin/sh -c 'rm /tmp/vscode-server-linux-x64.tar.gz'
