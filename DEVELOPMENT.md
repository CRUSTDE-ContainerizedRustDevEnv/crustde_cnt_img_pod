...
```bash
podman exec --user=root crustde_vscode_cnt  curl -L https://github.com/rui314/mold/releases/download/v2.32.1/mold-2.32.1-x86_64-linux.tar.gz --output /tmp/mold.tar.gz
...
```bash
cd  ~/rustprojects/crustde_cnt_img_pod/create_and_push_container_images
./ssh_auth_podman_push docker.io/bestiadev/crustde_cargo_img:cargo-1.79.0
./ssh_auth_podman_push docker.io/bestiadev/crustde_cargo_img:latest

./ssh_auth_podman_push docker.io/bestiadev/crustde_cross_img:cargo-1.79.0
./ssh_auth_podman_push docker.io/bestiadev/crustde_cross_img:latest

./ssh_auth_podman_push docker.io/bestiadev/crustde_vscode_img:vscode-1.90.2
./ssh_auth_podman_push docker.io/bestiadev/crustde_vscode_img:cargo-1.79.0
...
| ---------------------------------------- | -------------- |------------- | ----------- |
| docker.io/bestiadev/crustde_cargo_img   | cargo-1.79.0   | 1.28 GB      | 0.45 GB     |
| docker.io/bestiadev/crustde_cross_img   | cargo-1.79.0   | 3.03 GB      | 0.98 GB     |
| docker.io/bestiadev/crustde_vscode_img  | cargo-1.79.0   | 3.32 GB      | 1.06 GB     |
...
```bash
ssh -i ~/.ssh/github_com_bestia_dev_git_ssh_1 -p 2201 rustdevuser@localhost -v
```
...
To make the SSH client work in the container I need the file with the private key for SSH connection to GitHub. I already have this in the file `~/.ssh/github_com_bestia_dev_git_ssh_1`. I will copy it into the container with `podman cp`.  
