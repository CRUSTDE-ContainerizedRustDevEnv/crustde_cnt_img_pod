...
echo "\033[0;33m    Name of the image: rust_ts_dev_img \033[0m"
...
echo "\033[0;33m sh rust_ts_dev_img.sh \033[0m"
...
set -e
podman rm rust_ts_dev_cnt || :
buildah rm rust_ts_dev_img || :
buildah rmi -f docker.io/bestiadev/rust_ts_dev_img || :
...
buildah from --name rust_ts_dev_img docker.io/bestiadev/rust_dev_vscode_img:vscode-1.70.2

buildah config \
--author=github.com/bestia-dev \
--label name=rust_ts_dev_img \
...
buildah run --user root rust_ts_dev_img    apt -y update
buildah run --user root rust_ts_dev_img    apt -y upgrade

echo " "
echo "\033[0;33m    typescript compiler, nodejs, npm \033[0m"
buildah run --user root rust_ts_dev_img    apt install -y nodejs npm 
echo "\033[0;33m    node -v \033[0m"
buildah run --user root rust_ts_dev_img    node -v
# on 2022-07-19 is v12.22.12
echo "\033[0;33m    npm -v \033[0m"
buildah run --user root rust_ts_dev_img    npm -v
# on 2022-07-19 is 7.5.2
buildah run --user root rust_ts_dev_img    npm install -g typescript
echo "\033[0;33m    tsc --version \033[0m"
buildah run --user root rust_ts_dev_img    tsc --version
# on 2022-07-19 is 4.7.4

echo " "
echo "\033[0;33m    Remove unwanted files \033[0m"
buildah run --user root rust_ts_dev_img    apt -y autoremove
buildah run --user root rust_ts_dev_img    apt -y clean

echo " "
echo "\033[0;33m    Finally save/commit the image named rust_ts_dev_img \033[0m"
...
buildah tag docker.io/bestiadev/rust_ts_dev_img:latest docker.io/bestiadev/rust_ts_dev_img:vscode-1.70.2
buildah tag docker.io/bestiadev/rust_ts_dev_img:latest docker.io/bestiadev/rust_ts_dev_img:cargo-1.63.0
...
echo "\033[0;33m podman push docker.io/bestiadev/rust_ts_dev_img:vscode-1.70.2 \033[0m"
